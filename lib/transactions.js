...
TxController.prototype.transformTransaction = function (transaction, options, callback) {
    ...
    if (transaction.coinbase) {
        ...
        if (transformed.vin && transformed.vin.length) {
            for (var i = 0; i < transformed.vin.length; i++) {
                ...
                if (vin && vin.scriptSig && vin.scriptSig.asm && vin.scriptSig.asm.indexOf('OP_ZEROCOINSPEND ') > -1) {
                    vin.addr = 'Zerospend';
                } else if (vin && vin.scriptSig && vin.scriptSig.asm && vin.scriptSig.asm.indexOf('OP_ZEROCOINSPEND_V3 ') > -1) {
                    vin.addr = 'Sigmaspend';
                }
            }
        }
    ...
    if (transformed.vout && transformed.vout.length) {
        for (var o = 0; o < transformed.vout.length; o++) {
            ...
            if (vout && vout.scriptPubKey && vout.scriptPubKey.asm && vout.scriptPubKey.asm.indexOf('OP_ZEROCOINMINT ') > -1) {
                if (!vout.scriptPubKey.addresses) {
                    vout.scriptPubKey.addresses = [];
                }
                vout.scriptPubKey.type = 'zerocoinmint';
                vout.scriptPubKey.addresses.push('Zeromint');
            } else if (vout && vout.scriptPubKey && vout.scriptPubKey.asm && vout.scriptPubKey.asm.indexOf('OP_ZEROCOINMINT_V3 ') > -1) {
                if (!vout.scriptPubKey.addresses) {
                    vout.scriptPubKey.addresses = [];
                }
